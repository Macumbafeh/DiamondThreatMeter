...
local function ColorPickerCallback(restore)
	...
	Robin_SetColor(colorTarget, newR, newG, newB)
end
local function ShowColorPicker(r, g, b, a)
	...
	ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = ColorPickerCallback, ColorPickerCallback, ColorPickerCallback
...
function DTM_Console_OnCommand(input, feedback)
	...
	if string.find(input, "color ") ~= nil then
		if input == "color 1" then
			colorTarget = 1
		elseif input == "color 2" then
			colorTarget = 2
		elseif input == "color 3" then
			colorTarget = 3
		elseif input == "color 4" then
			colorTarget = 4
		end

		DEFAULT_CHAT_FRAME:AddMessage("Changing color for: " .. Robin_GetColorName(colorTarget))

		local color = Robin_GetColor(colorTarget)
		ShowColorPicker(color[1], color[2], color[3], 1)
	end
