local backgroundColor = {0.101, 0.101, 0.101, 1.0}
local backgroundBorderColor = {0, 0, 0, 1.0}
...
local function ApplyCommonSkin(listFrame)
  ...
  --else
    ...
    listFrame.tileTexture = "Interface\\AddOns\\DiamondThreatMeter\\gfx\\BackgroundTexture";
    --listFrame.tileTexture = "Interface\\Tooltips\\UI-Tooltip-Background";
    listFrame.edgeTexture = "Interface\\AddOns\\DiamondThreatMeter\\gfx\\BackgroundTexture"
    listFrame.backdrop.edgeSize = 1;
	...
	--local edge = CreateFrame("Frame", nil, listFrame)
	--edge:SetFrameLevel(listFrame:GetFrameLevel() - 1)
	--edge:SetPoint("BOTTOMLEFT", listFrame, -1, -1)
	--edge:SetPoint("TOPRIGHT", listFrame, 1, 1)
	--local texture1 = edge:CreateTexture("ARTWORK")
	--texture1:SetAllPoints()
	--texture1:SetTexture(backgroundBorderColor[1], backgroundBorderColor[2], backgroundBorderColor[3], backgroundBorderColor[4])
	--edge.texture = texture1

    listFrame.backdrop.bgFile = listFrame.tileTexture;
    listFrame.backdrop.edgeFile = listFrame.edgeTexture;
    listFrame.backdrop.insets.left = 0;
    listFrame.backdrop.insets.right = 0;
    listFrame.backdrop.insets.top = 0;
    listFrame.backdrop.insets.bottom = 0;
    ...
    listFrame:SetBackdropBorderColor(backgroundBorderColor[1], backgroundBorderColor[2], backgroundBorderColor[3], backgroundBorderColor[4]);
    listFrame:SetBackdropColor(backgroundColor[1], backgroundColor[2], backgroundColor[3], backgroundColor[4]);
end
...
function DTM_CommonListFrame_OnUpdate(self, elapsed)
    ...
    if ( self.targetHeight ) and ( self.height ~= self.targetHeight ) then
		...
		local diff = self.targetHeight - self.height
