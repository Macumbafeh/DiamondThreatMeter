...
selfPlayerColor = {0, 0.5882352941176471, 1}
otherPlayerColor = {0, 0.592156862745098, 0.1019607843137255}
dangerColor = {1, 0.196078431372549, 0.1490196078431373}
warningColor = {0.9019607843137255, 0.6901960784313725, 0}

function Robin_ForceUpdateColor(self)
	if UnitName("player") == self.name then
		self:SetStatusBarColor(selfPlayerColor[1], selfPlayerColor[2], selfPlayerColor[3], 1)
	else
		self:SetStatusBarColor(otherPlayerColor[1], otherPlayerColor[2], otherPlayerColor[3], 1)
	end
end

function Robin_ForceUpdateColorsOnRow(row)
	Robin_ForceUpdateColor(row)
	row.dangerRedBar:SetStatusBarColor(dangerColor[1], dangerColor[2], dangerColor[3], 1);
	row.dangerYellowBar:SetStatusBarColor(warningColor[1], warningColor[2], warningColor[3], 1);
end

robin_rowList = {}
function Robin_ForceUpdateAllColors()
    for i=1, #robin_rowList do
		Robin_ForceUpdateColorsOnRow(robin_rowList[i])
    end
end

function Robin_GetColorName(index)
	if index == 1 then
		return "Self player color"
	elseif index == 2 then
		return "Other player color"
	elseif index == 3 then
		return "Threat color"
	elseif index == 4 then
...
function Robin_SetColor(index, r, g, b)
	if index == 1 then
		selfPlayerColor = {r, g, b}
		DTM_Skins[Robin_GetColorName(index)] = selfPlayerColor
	elseif index == 2 then
		otherPlayerColor = {r, g, b}
		DTM_Skins[Robin_GetColorName(index)] = otherPlayerColor
	elseif index == 3 then
		dangerColor = {r, g, b}
		DTM_Skins[Robin_GetColorName(index)] = dangerColor
	elseif index == 4 then
		warningColor = {r, g, b}
		DTM_Skins[Robin_GetColorName(index)] = warningColor
	end

	Robin_ForceUpdateAllColors()
end
...
local function ValidateGetLineOfSight(isInLineOfSight)
	local fixedBoolStatus = FixBoolStatus(isInLineOfSight)
	if fixedBoolStatus == "65327832663285" or
	   fixedBoolStatus == "10510810811710910511097116101" then
		return true
	end
...
local ANIMATION_LENGTH_WARNING_START = 0.001;
local ANIMATION_LENGTH_WARNING_END = 0.001;

local LIGHTING_ANIMATION_DELAY = 0.0;
local LIGHTING_ANIMATION_DURATION = 0.0;
...
local function Activate(row, name, guid, threat, class, aggroThreat, position, hasAggro, tps)
    if ( row.status == "UNUSED" ) then
		...
		Robin_ForceUpdateColorsOnRow(row)
		table.insert(robin_rowList, row)
...
local asjhdjkasndkj = false

function DTM_ThreatListRow_OnUpdate(self, elapsed)
	-- Loop through all custom colors
    for i=1, 4 do
		-- Check if we have a saved color
		local savedColor = DTM_Skins[Robin_GetColorName(i)]
		local currentColor = Robin_GetColor(i)
		if savedColor ~= nil and currentColor ~= nil then
			-- Check if the current color is not the saved color
			if currentColor[1] ~= savedColor[1] or
			   currentColor[2] ~= savedColor[2] or 
			   currentColor[3] ~= savedColor[3] then
				-- Update the color to the saved one
				--DEFAULT_CHAT_FRAME:AddMessage("Found an updated color!")
				Robin_SetColor(i, savedColor[1], savedColor[2], savedColor[3])
			end
		end
	end
    ...
    if ( self.positionStatus == "STARTWARNING" or self.positionStatus == "WARNING" ) then
        ...
        baseY = self:GetParent():GetTop() - 42 - (self.position-1)*16.5 - 8;
        baseZ = 1.0;
    end
